...
def check_engine():
    engine_path = os.path.join(get_mine_studio_dir(), "engine", "build", "libs", "mcprec-6.13.jar")
    if not os.path.exists(engine_path):
        # Check if we're in a non-interactive environment (Docker, CI, etc.)
        # If stdin is not a TTY, automatically download
        import sys
        is_interactive = sys.stdin.isatty() if hasattr(sys.stdin, 'isatty') else False
        
        if not is_interactive or os.environ.get('AUTO_DOWNLOAD_ENGINE', '').lower() in ('1', 'true', 'yes'):
...
class MinecraftSim(gymnasium.Env):
    ...
    def __init__(
        ...
        max_mem: Optional[str] = None,                  # Java max memory (e.g., "2G"), overrides MINECRAFT_MAX_MEM env var
        **kwargs
    ) -> Any:
        super().__init__()
        ...
        # Set max_mem environment variable if provided
        if max_mem is not None:
            os.environ["MINECRAFT_MAX_MEM"] = max_mem
